...
public class Question_5_A {
    ...
    private static int[] generateInitialSolution() {
        int[] solution = new int[Question_5_A.NUM_CITIES];
        for (int i = 0; i < Question_5_A.NUM_CITIES; i++) {
            solution[i] = i;
        }

        Random rand = new Random();
        for (int i = 0; i < Question_5_A.NUM_CITIES; i++) {
            int j = rand.nextInt(Question_5_A.NUM_CITIES);
            int temp = solution[i];
    ...
    private static int calculateTotalDistance(int[] solution) {
        ...
        for (int i = 0; i < solution.length - 1; i++) {
            totalDistance += Question_5_A.DISTANCE_MATRIX[solution[i]][solution[i + 1]];
        }

        totalDistance += Question_5_A.DISTANCE_MATRIX[solution[solution.length - 1]][solution[0]];
